...
def build_dimer_records(
    *,
    ...
    """Build dimer records for all unique fragment pairs (FMO2 pairwise terms).

    For each pair (i, j) with i < j, the dimer geometry is the union of the
    two fragments' atom indices.  The active space is inferred from the dimer
    geometry using the same logic as ``build_fragment_records``.

    Returns a list of dimer records, each with keys:
        name, parent_name, frag_i, frag_j, geometry, charge, multiplicity,
        basis, atom_indices, active_space, parent_active_space.
    """
    dimer_records: list[dict[str, Any]] = []
    n_frags = len(fragment_records)
    for i in range(n_frags):
        for j in range(i + 1, n_frags):
            frag_i = fragment_records[i]
            frag_j = fragment_records[j]
            atoms_i = frag_i.get("atom_indices", [])
            atoms_j = frag_j.get("atom_indices", [])
            dimer_atoms = sorted(set(atoms_i) | set(atoms_j))
            dimer_geom = fragment_geometry(geometry, dimer_atoms)

            dimer_charge = int(frag_i.get("charge", charge)) + int(frag_j.get("charge", charge))

            dimer_active_space = build_active_space_spec(
                geometry=dimer_geom,
                basis=basis,
                charge=dimer_charge,
                multiplicity=multiplicity,
            ...
            name_i = frag_i.get("name", f"frag_{i}")
            name_j = frag_j.get("name", f"frag_{j}")
            dimer_name = f"dim_{name_i}_{name_j}"

            dimer_records.append({
                "name": dimer_name,
                "parent_name": parent_name,
                "frag_i": i,
                "frag_j": j,
                ...
                "multiplicity": multiplicity,
                "basis": basis,
                "atom_indices": dimer_atoms,
                "active_space": dimer_active_space.as_dict(),
                "parent_active_space": None if active_space is None else active_space.as_dict(),
            })
