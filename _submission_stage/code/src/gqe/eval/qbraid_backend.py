...
def _circuit_to_qasm(circuit: Any) -> str:
    """Export a Qiskit circuit to QASM2 string, compatible across Qiskit versions.

    Qiskit >= 1.0 removed the ``QuantumCircuit.qasm()`` method in favor of
    ``qiskit.qasm2.dump``.  This helper tries the new API first and falls
    ...
    try:
        ...
        pass
    # Last resort: str representation (not valid QASM but preserves hash)
    return str(circuit)
...
def _group_qwc_terms(active_terms: list[tuple[str, float]]) -> list[list[int]]:
    ...
    Uses vectorized bit-manipulation via src.gqe.accel.fast_qwc for O(n²)
    NumPy/GPU computation instead of character-by-character Python loops.
    ...
    *active_terms*.
    ...
    try:
        ...
        return group_qwc_terms_vectorized(active_terms, use_gpu=True)
    except Exception:
        pass
...
def _parse_grouped_results(
    ...
    Uses GPU-accelerated parity computation via src.gqe.accel.gpu_parity
    (Triton kernel or PyTorch GPU) with NumPy vectorized fallback.
    """
    # Try accelerated path
    try:
        from src.gqe.accel.gpu_parity import parse_grouped_results_gpu
        return parse_grouped_results_gpu(results, group_mapping, n_qubits, shots, use_triton=True)
    except Exception:
        pass
...
def export_sqd_sampling_circuit(
    molecule_record: dict[str, Any],
    operators: list[str],
    theta_values: np.ndarray | None = None,
    device: str = "aws:rigetti:qpu:cepheus-1-108q",
    shots: int = 4096,
    out_path: Path | None = None,
) -> dict[str, Any]:
    """Export a computational-basis measurement circuit for SQD sampling.

    This builds the H-cGQE ansatz circuit, binds the rotation parameters,
    adds computational-basis measurements, and produces a manifest that can
    be submitted to any QPU. No credits are spent — this is export-only.

    Args:
        molecule_record: Hamiltonian record.
        operators: H-cGQE operator sequence.
        theta_values: Rotation angles. If None, uses zeros.
        device: Target qBraid device ID.
        shots: Number of shots for the sampling run.
        out_path: Optional path to write the manifest JSON.

    Returns:
        Manifest dict with circuit QASM, metadata, and SQD pipeline info.
    """
    _needs_qiskit()
    ...
    bound_circuit = circuit.assign_parameters(
        {t: float(v) for t, v in zip(thetas, theta_values)}
    )
    bound_circuit.measure_all()

    # Generate QASM for portability
    qasm_str = _circuit_to_qasm(bound_circuit)

    # Compute circuit hash for ledger idempotency
    import hashlib
    circuit_hash = "sha256:" + hashlib.sha256(qasm_str.encode()).hexdigest()

    manifest = {
        "export_type": "sqd_sampling",
        "measurement_basis": "computational_basis",
        "molecule": molecule_record.get("name", "unknown"),
        "n_qubits": n_qubits,
        "n_electrons": n_electrons,
        "n_operators": len(operators),
        "operators": operators,
        "thetas": [float(v) for v in theta_values],
        "device": device,
        "shots": shots,
        "circuit_hash": circuit_hash,
        "circuit_qasm": qasm_str,
        "circuit_depth": bound_circuit.depth(),
        "circuit_gates": dict(bound_circuit.count_ops()),
        "n_hamiltonian_terms": len(molecule_record.get("terms", [])),
        "pipeline_stage": "sqd_pilot",
        "export_timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "description": (
            "Computational-basis sampling circuit for SQD. "
            "Submit to QPU, collect counts, then run sqd_energy_from_counts()."
        ),
    }
    ...
    if out_path is not None:
        ...
        print(f"SQD sampling manifest exported: {out_path}")

    return manifest


def export_qwc_diagnostic_circuits(
    molecule_record: dict[str, Any],
    operators: list[str],
    theta_values: np.ndarray | None = None,
    device: str = "aws:rigetti:qpu:cepheus-1-108q",
    ...
    """Export QWC-grouped circuits for independent energy estimation.

    This builds the H-cGQE ansatz circuit, groups Hamiltonian Pauli terms
    by qubit-wise commutativity, creates one measurement circuit per group,
    and produces a manifest for batch QPU submission. No credits are spent.

    The QWC path is independent from the SQD path:
    - SQD: computational-basis counts -> subspace diagonalization (variational)
    - QWC: per-group expectation values -> direct energy estimate (non-variational)
    ...
    Args:
        ...
        theta_values: Rotation angles. If None, uses zeros.
        device: Target qBraid device ID.
        shots: Number of shots per group circuit.
        out_path: Optional path to write the manifest JSON.

    Returns:
        Manifest dict with grouped circuits, QASM, and QWC pipeline info.
    """
    _needs_qiskit()
    ...
    # Build per-group circuits
    group_circuits = []
    for gi, group_indices in enumerate(groups):
        group_circuit = bound_circuit.copy()
        # Determine measurement basis for this group
        basis = ["I"] * n_qubits
        for idx in group_indices:
            word, _ = active_terms[idx]
            for q in range(len(word)):
                if word[q] != "I" and basis[q] == "I":
                    basis[q] = word[q]

        # Apply basis changes and measure
        for q in range(n_qubits):
            q_qiskit = n_qubits - 1 - q
            if basis[q] == "X":
                group_circuit.h(q_qiskit)
            elif basis[q] == "Y":
                group_circuit.sdg(q_qiskit)
                group_circuit.h(q_qiskit)

        group_circuit.measure_all()

        # Build group metadata
        group_terms = []
        for idx in group_indices:
            word, coeff = active_terms[idx]
            group_terms.append({
                "pauli_word": word,
                "coefficient": coeff,
                "term_index": idx,
        ...
        group_circuits.append({
            ...
            "terms": group_terms,
            "circuit_qasm": _circuit_to_qasm(group_circuit),
            "circuit_depth": group_circuit.depth(),
        })

    # Compute combined circuit hash for ledger
    import hashlib
    combined_hash = "sha256:" + hashlib.sha256(
        "".join(gc["circuit_qasm"] for gc in group_circuits).encode()
    ).hexdigest()

    manifest = {
        "export_type": "qwc_diagnostics",
        "measurement_basis": "qwc_grouped",
        "molecule": molecule_record.get("name", "unknown"),
        "n_qubits": n_qubits,
        "n_electrons": n_electrons,
        "n_operators": len(operators),
        ...
        "device": device,
        "shots": shots,
        "circuit_hash": combined_hash,
        "n_groups": len(groups),
        "n_hamiltonian_terms": len(active_terms),
        "group_circuits": group_circuits,
        "pipeline_stage": "qwc_diagnostics",
        "export_timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "description": (
            "QWC-grouped measurement circuits for independent energy estimation. "
            "Submit each group circuit to QPU, collect counts, then compute "
            "per-term expectations and sum for total energy."
        ),
    }
    ...
    if out_path is not None:
        ...
        with out_path.open("w", encoding="utf-8") as f:
            json.dump(manifest, f, indent=2)
        print(f"QWC diagnostic manifest exported: {out_path} ({len(groups)} groups)")

    return manifest


def submit_via_ledger(
    manifest: dict[str, Any],
    ledger_db_path: Path | str,
    job_id: str,
    budget: float | None = None,
) -> dict[str, Any]:
    """Submit a QPU job through the durable ledger with idempotency and budget checks.

    Args:
        manifest: Export manifest from export_sqd_sampling_circuit or export_qwc_diagnostic_circuits.
        ledger_db_path: Path to the SQLite ledger database.
        job_id: QPU job ID to assign.
        budget: Optional credit budget for enforcement.

    Returns:
        Ledger entry for the submitted job.
    """
    from src.gqe.eval.qpu_ledger import QpuLedger, LedgerEntry

    ledger = QpuLedger(ledger_db_path)
    if budget is not None:
        ledger.set_budget(budget)

    entry = LedgerEntry(
        circuit_hash=manifest["circuit_hash"],
        molecule=manifest["molecule"],
        device_id=manifest["device"],
        shots=manifest["shots"],
        n_qubits=manifest["n_qubits"],
        n_circuits=manifest.get("n_groups", 1),
        measurement_basis=manifest["measurement_basis"],
        pipeline_stage=manifest["pipeline_stage"],
        metadata={
            "operators": manifest["operators"],
            "thetas": manifest["thetas"],
            "export_type": manifest["export_type"],
        },
    )

    result = ledger.submit(entry, job_id=job_id)
    print(f"Ledger entry: job_id={result.job_id}, status={result.status.value}, "
          f"est_cost={result.estimated_cost_credits}")
    ledger.close()
    return {
        "job_id": result.job_id,
        "status": result.status.value,
        "estimated_cost_credits": result.estimated_cost_credits,
...
def main() -> None:
    ...
    parser.add_argument("--export-sqd", action="store_true", help="Export SQD sampling manifest (no credits spent)")
    parser.add_argument("--export-qwc", action="store_true", help="Export QWC diagnostic manifest (no credits spent)")
    parser.add_argument("--ledger-db", type=Path, default=None, help="SQLite ledger DB path for job tracking")
    parser.add_argument("--budget", type=float, default=None, help="Credit budget for ledger enforcement")
    ...
    # Export Mode (no credits spent)
    if args.export_sqd or args.export_qwc:
        if not (args.hamiltonians and args.molecule and args.out):
            parser.print_help()
            sys.exit("\nError: --hamiltonians, --molecule, and --out are required for export.")

        records = load_hamiltonian_records(args.hamiltonians)
        record = find_record_by_name(records, args.molecule)
        if record is None:
            sys.exit(f"\nError: molecule '{args.molecule}' not found in Hamiltonian data.")

        # Load operators from optimized results if available
        operators = []
        thetas = []
        if args.optimized and args.optimized.exists():
            ...
            for entry in optimized.get("results", []):
                if entry.get("molecule") == args.molecule:
                    best_seq = entry.get("best_sequence", entry)
                    operators = best_seq.get("best_operators", best_seq.get("operators", []))
                    thetas = best_seq.get("best_thetas", best_seq.get("thetas", []))
                    break

        if not operators:
            print(f"WARNING: No operators found for {args.molecule}, using empty sequence (HF-only).")

        if args.export_sqd:
            manifest = export_sqd_sampling_circuit(
                record, operators,
                theta_values=np.asarray(thetas) if thetas else None,
                device=args.device,
                shots=args.shots,
                out_path=args.out,
            )
        ...
        # Optionally register with ledger
        if args.ledger_db:
            submit_via_ledger(
                manifest,
                ledger_db_path=args.ledger_db,
                job_id=f"export_{manifest['molecule']}_{int(time.time())}",
                budget=args.budget,
            )
        return
