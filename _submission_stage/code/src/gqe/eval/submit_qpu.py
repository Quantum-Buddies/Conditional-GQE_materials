...
try:
    from src.gqe.eval.qbraid_backend import _build_ansatz_circuit as _build_canonical_circuit
    from src.gqe.eval.qbraid_backend import _group_qwc_terms, _build_grouped_circuits
    _CANONICAL_BUILDER_AVAILABLE = True
except ImportError:
    _CANONICAL_BUILDER_AVAILABLE = False
...
def _build_qiskit_circuit(
    ...
    Delegates to the canonical builder in qbraid_backend.py to ensure
    circuit construction is consistent across all submission paths.
    Falls back to local construction if the canonical builder is unavailable.
    """
    if _CANONICAL_BUILDER_AVAILABLE:
        import numpy as np
        theta_arr = np.array(thetas) if thetas is not None else None
        return _build_canonical_circuit(n_qubits, n_electrons, operators, theta_arr)

    # Fallback: local construction (kept for environments without qiskit extras)
