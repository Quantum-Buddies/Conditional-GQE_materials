...
def retrieve_job_counts(job_id: str, reverse_bits: bool = False, n_qubits: int = 0) -> dict[str, int] | None:
...
def run_sqd_on_counts(record: dict, counts: dict[str, int], n_electrons: int, reverse_bit_order: bool = False) -> dict:
...
def main() -> None:
    ...
    for mol, job_id in sqd_jobs.items():
        ...
        sqd_result = run_sqd_on_counts(record, counts, n_electrons, reverse_bit_order=args.reverse_bits)
        all_results[mol] = {
            "status": "completed",
            "counts": counts,
