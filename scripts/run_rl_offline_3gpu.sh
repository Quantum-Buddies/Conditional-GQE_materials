#!/bin/bash
# Offline RL Training (cache-only, 3 GPU, max throughput) — portable, no Slurm
# Usage: bash scripts/run_rl_offline_3gpu.sh
set -euo pipefail
...
OUTPUT="results/train/h_cgqe_model_rl_offline_3gpu.pt"

echo "=== Offline RL Training (3 GPU, cache-only mode) ==="
echo "Start: $(date)"
echo "GPUs: $(nvidia-smi -L 2>/dev/null | wc -l)"

if [ ! -f "$ENERGY_CACHE" ] || [ ! -f "$SFT_CHECKPOINT" ]; then
    echo "ERROR: Need rl_energy_cache.sqlite and h_cgqe_model_b200_sft.pt. Run: git lfs pull"
    exit 1
fi
...
"${PYTHON}" -u src/gqe/models/train_rl_dapo.py \
    ...
    --out "${OUTPUT}" --epochs 300 --n-samples 64 --n-iters 4 --lr 1e-5 \
