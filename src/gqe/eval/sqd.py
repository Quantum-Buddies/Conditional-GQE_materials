...
def sqd_energy_from_counts(
    ...
    Pipeline:
    1. Optionally reverse bitstrings for QPU backends with opposite bit ordering
    2. Select bitstrings by frequency (most frequent first)
    3. Optionally filter by symmetry (particle number, spin parity)
    4. Build subspace Hamiltonian and diagonalize
    ...
    Args:
        ...
        reverse_bit_order: If True, reverse bitstring keys (for Rigetti/QBraid
            QPU results where qubit 0 is leftmost instead of rightmost).
...
def reverse_bitstrings_in_counts(
    counts: Dict[str, int],
    n_qubits: int,
) -> Dict[str, int]:
    """Reverse bitstring keys in a counts dict to convert between bit orderings.

    Some QPU backends (e.g. Rigetti via qBraid) return bitstrings with qubit 0
    as the LEFTMOST character, while our SQD convention (matching Qiskit) uses
    qubit 0 as the RIGHTMOST character (LSB). This function reverses each
    bitstring key to convert between conventions.

    Args:
        counts: Raw counts dict with bitstring keys.
        n_qubits: Number of qubits (for zero-padding before reversal).

    Returns:
        New counts dict with reversed bitstring keys.
    """
    ...
    for bs, count in counts.items():
        ...
        reversed_bs = bs[::-1]
        reversed_counts[reversed_bs] = reversed_counts.get(reversed_bs, 0) + int(count)
...
def canonicalize_counts(
    ...
    - Optionally reversed for backends that use opposite bit ordering
...
def run_sqd(
    ...
    1. Canonicalize counts (optionally reversing bit order for QPU backends)
    ...
    # Step 1: Canonicalize (with optional bit order reversal for QPU backends)
    counts = canonicalize_counts(counts, n_qubits, reverse_bit_order=reverse_bit_order)
